...
- June 28, 2025. Updated WhatsApp and PDF invoice templates to match requested tabular format - Enhanced WhatsApp message templates with mobile-friendly bullet point format replacing broken pipe tables, improved readability with emojis and proper spacing. Streamlined PDF invoice templates to show only Order Summary table with status information, removed detailed Order Items section as requested. Both templates maintain existing payment tracking logic and business calculations while providing cleaner, more professional presentation
- June 28, 2025. Implemented detailed Order Items table format in InvoiceTemplate and SimplePDFService - Added comprehensive Order Items table showing Item, Quantity (kg), Rate (₹), Amount columns exactly matching user's reference image format. InvoiceTemplate displays detailed breakdown per order with mobile-responsive design. SimplePDFService includes both detailed Order Items table and Order Summary with status columns for complete invoice presentation. WhatsApp templates remain unchanged as confirmed by user
